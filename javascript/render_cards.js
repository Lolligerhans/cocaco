...
class RenderCards {
     ...
     * @type {Toggle}
     */
    #toggle = new Toggle("resourcesPanel", "rollsPlot", "logPanel")
                  .enable("resourcesPanel", "logPanel");
    ...
    /**
     * Hide or unhide the log panel that the main script sets up. It is quite
     * unfortunate that the code for that now lives here. Given we want to keep
     * the buttons next to each other, maybe define a callback for buttons that
     * the render code does not care but the button is still generated by the
     * renderer. Or Just always show the log. Has fewer downsides (?).
     */
    #updateLogPanel() {
        // TODO: Pass that element to the ctor so we dont duplicate the
        //       variation points.
        let log = document.querySelector(".cocaco.cocaco-log");
        console.assert(log, "Cannot toggle log panel: no log panel found");
        if (this.#toggle.isToggled("logPanel")) {
            log.classList.remove("cocaco-gone");
        } else {
            log.classList.add("cocaco-gone");
        }
    }
    ...
    static colonistAssetMap = (_path, resource) => {
        const newPath = `https://cdn.colonist.io/dist/assets/${
            Colonist.imageNameSnippets[resource]}.${
            Colonist.imageNameAdditions[resource]}.svg`;
        return newPath;
    };
